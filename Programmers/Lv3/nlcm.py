...
정의를 확장해서, n개의 수의 최소공배수는 n 개의 수들의 배수 중 공통이 되는 가장 작은 숫자가 됩니다. nlcm 함수를 통해 n개의 숫자가 입력되었을 때,
최소공배수를 반환해 주세요. 예를들어 [2,6,8,14] 가 입력된다면 168을 반환해 주면 됩니다.
'''

'''
[ 접근방법 ]
입력받은 N개의 숫자는 순서대로 소인수분해 하여 임시변수 tempDic에 담았다. 그 후 해당 지수의 가장 큰 차수를 찾기 위해 answer에 담긴 지수와 tempDic에
담긴 지수를 비교해 차수가 높은 것을 answer에 담고, 마지막으로 answer에 담긴 지수를 각각의 차수만큼 곱한 값을 모두 곱해 리턴해서 풀었다.

사실 gcd와 lcm의 관계만 충분히 숙지했으면 더 쉽게 풀 수 있었는데 하는 아쉬움이 있다.
'''
