...
하지만 시간복잡도를 줄이기 위해 식을 더 단순화 할수 없을까 고민하다 보니 n이 홀수던 짝수이던 n//2 + 1이 최대 범위라는 것을 알아냈다. 
예를 들어 15의 경우 15//2 + 1 = 8보다 더 큰 연속된 수는 2개만 더해도 15를 넘어간다. (ex 8 + 9 = 17)
따라서 기존에 num까지 구했던 경우를 num // 2 + 1 까지 구하면 시간복잡도를 반으로 줄일 수 있다.
