...
'''
[ 접근방법 ]
처음으로 dp와 재귀의 차이를 알게 한 문제다. 우리는 정사각형을 찾아야 한다. 한 변의 길이가 1인 정사각형은 한 값만 O이면 된다.
그럼 한 변의 길이가 2인 정사각형은? 여기서부턴 대상의 값 뿐만 아니라 근처의 값도 확인해야 한다. 이 알고리즘은 dp에 각 좌표가 넓이가 얼마인지 기록하게 된다. 
또한 한 변의 최소 길이를 2로 정의한다.(1은 사실 의미가 없다)

그럼 2보다 큰 정사각형을 찾아보자. 이때 기준을 (0,0)부터 시작해서 (n-1, n-1)까지 할지, (1,1) 부터 (n, n)까지 할지 기준을 잘 정해야 한다.
강의에선 후자로 했기 때문에 우리도 후자로 한다.

(1,1)은 최대 크기가 2이다. (우리가 기준으로 삼는 꼭지점은 정사각형의 우측 아래 꼭지점이라는 것을 기억해야 한다)
...
이런 것이다. 실제로 dp의 값을 출력해보면 입력된 값보다 좌표가 (+1, +1)씩 되어있다. 그러나 우리가 구하는 것은 최대값이고, 
for문을 돌면서 최대값과 구해진 dp의 값을 비교해 큰 값을 answer에 담고있기 때문에 좌표가 어긋나는 것을 신경쓰지 않아도 된다.
